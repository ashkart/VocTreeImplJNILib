...
public:
  ...
  /**
   * Stores the database in a file
   * @param connection_base PGSQL connection
   * @param recordName is name of record of data of images set
   */
  void saveToPG(pqxx::connection_base &conn, const string &recordName) const;
...
protected:
  ...
  struct IFPair
  {
     ...
     * @param eid entry id
     * @param wv word weight
     * @param img_name is name of the image recording into db
     */
    IFPair(EntryId eid, WordValue wv, std::string img_name): entry_id(eid), word_weight(wv) {image_name = img_name;}
...
protected:
  ...
  virtual void createTables(pqxx::connection_base &conn) const; 
  ...
  BowVector *bowvec, FeatureVector *fvec)
{
  ...
  if(m_use_di && fvec != NULL)
  {
    ...
    return add(v, img_name, *fvec);
  }
  else if(m_use_di)
  {
    FeatureVector fv;
    m_voc->transform(features, v, fv, m_dilevels); // with features
    return add(v, img_name, fv);
  }
  else if(fvec != NULL)
  {
    m_voc->transform(features, v, *fvec, m_dilevels); // with features
    return add(v, img_name);
  }
  else
  {
  ...
  const FeatureVector &fv)
{
  ...
  for(vit = v.begin(); vit != v.end(); ++vit)
  {
    ...
    ifrow.push_back(IFPair(entry_id, word_weight, img_name));
  }
  ...
  QueryResults &ret, int max_results, int max_id) const
{
  ...
  HashMap<EntryId, std::string, MyKeyHash> names;
  ...
  for(vit = vec.begin(); vit != vec.end(); ++vit)
  {
    ...
    for(rit = row.begin(); rit != row.end(); ++rit)
    {
      ...
      if((int)entry_id < max_id || max_id == -1)
      {
        ...
        std::string name = rit->image_name;
        ...
        else
        {
          ...
          names.put(entry_id, rit->image_name);
        }
      }
  ...
  {
      string name;
      names.get(pit->first, name);
    ret.push_back(Result(pit->first, pit->second, name));
  }
...
template<class TDescriptor, class F>
void TemplatedDatabase<TDescriptor, F>::saveToPG(pqxx::connection_base& conn, 
const string& recordName) const
{
    ...
    try {
        string statementName = "dataCheckStmt";
        work wCheck(conn, "CheckTables");
        conn.prepare(statementName, "select tablename from pg_tables "
        "where tablename = $1 or tablename = $2");
        result res = ((invocation)wCheck.prepared(statementName)
                (dataTableName)(imageDBTableName)).exec();
        wCheck.abort();
        if (res.size() < 2)
        {
            createTables(conn);
...
createTables(pqxx::connection_base& conn) const
{
    try {
        ...
        {
            std::cerr << "Error on creating table " << dataTableName << ":\n" << e.base().what() << std::endl;
        }
        ...
        try {
            ...
            std::cerr << "Error on creating table " << imageDBTableName << ":\n" << e.base().what() << std::endl;
        }
        
        ww.commit();
    } catch (const pqxx::pqxx_exception &e) {
        std::cerr << "Error on creating tables:\n" << e.base().what() << std::endl;
    }
}
...
{ 
  ...
  for(WordId wid = 0; wid < fn.size(); ++wid)
  {
    ...
    for(unsigned int i = 0; i < fw.size(); ++i)
    {
      ...
      std::string img_name = fw[i]["image_name"];
